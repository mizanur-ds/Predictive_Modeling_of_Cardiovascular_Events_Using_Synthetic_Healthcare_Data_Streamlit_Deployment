...
def train_models():
    ...
    acc_tree = accuracy_score(y_valid, tree_model.predict(X_valid))
    model_scores["Decision Tree"] = acc_tree

    # Random Forest
    forest_model = RandomForestClassifier(random_state=42)
    forest_model.fit(X_train, y_train)
    acc_forest = accuracy_score(y_valid, forest_model.predict(X_valid))
    model_scores["Random Forest"] = acc_forest
    ...
    nn_model.fit(X_train_scaled, y_train, epochs=5, verbose=0)
    y_pred_nn = nn_model.predict(X_valid_scaled)
    y_pred_nn = [np.argmax(i) for i in y_pred_nn]
    acc_nn = accuracy_score(y_valid, y_pred_nn)
    model_scores["Neural Network"] = acc_nn
...
def main():
    ...
    st.markdown("Step-by-step feature input using the best model (Logistic Regression).")

    model, feature_names, model_scores = train_models()

    st.subheader("Model Accuracy Comparison")
    st.table(pd.DataFrame(model_scores.items(), columns=["Model", "Accuracy"]).sort_values(by="Accuracy", ascending=False))
